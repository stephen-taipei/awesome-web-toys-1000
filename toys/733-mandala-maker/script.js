...
function drawLine(x1, y1, x2, y2) {
    ...
    for (let i = 0; i < divisions; i++) {
        ...
        // Mirror
        const mx1 = centerX + dx1 * Math.cos(angle) + dy1 * Math.sin(angle);
        const my1 = centerY + dx1 * Math.sin(angle) - dy1 * Math.cos(angle);
        const mx2 = centerX + dx2 * Math.cos(angle) + dy2 * Math.sin(angle);
        const my2 = centerY + dx2 * Math.sin(angle) - dy2 * Math.cos(angle);
...
divisionsInput.addEventListener('input', () => {
    divisions = parseInt(divisionsInput.value);
    divVal.textContent = divisions;
});
